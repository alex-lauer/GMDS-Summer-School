...
In general, these simple approaches are not recommended for use. Methods are of historic interest and provide a useful starting point. Here, we consider two simple approaches.
...
Reduce the data set selecting only those subjects with observed outcome value(s). Creates selection bias, may cause overestimation of within group effects particularly at the last scheduled visit.
...
-   Look at the MMRM and at the complete case (CC) analysis again (from section missingness).
